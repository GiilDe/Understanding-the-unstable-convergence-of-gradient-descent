...
from utilities import get_gd_optimizer, get_gd_directory, get_loss_and_acc, compute_losses, \
    ...
    get_RP, get_directional_smoothness
...
def main(dataset: str, arch_id: str, loss: str, opt: str, lr: float, max_steps: int, neigs: int = 0,
         ...
         loss_goal: float = None, acc_goal: float = None, abridged_size: int = 5000, seed: int = 0, RP_DS_freq: int = -1):
    ...
    RPs = torch.zeros(max_steps // RP_DS_freq if RP_DS_freq >= 0 else 0, 1)
    directional_smoothness = torch.zeros(max_steps // RP_DS_freq if RP_DS_freq >= 0 else 0, 1)

    prev_loss = None
    for step in range(0, max_steps):
        train_loss[step], train_acc[step] = compute_losses(network, [loss_fn, acc_fn], train_dataset,
                                                           physical_batch_size, cuda)
        ...
        if eig_freq != -1 and step % eig_freq == 0:
            eigs[step // eig_freq, :] = get_hessian_eigenvalues(network, loss_fn, abridged_train, neigs=neigs,
                                                                physical_batch_size=physical_batch_size, cuda=cuda)
        ...
        optimizer.zero_grad()
        if RP_DS_freq != -1 and (step % RP_DS_freq == 1 or step % RP_DS_freq == 0):
            prev_parameters = parameters_to_vector(network.parameters())
        for (X, y) in iterate_dataset(train_dataset, physical_batch_size, cuda):
            if cuda:
                X = X.cuda()
                y = y.cuda()
            ...
            loss.backward()
        optimizer.step()
        if RP_DS_freq != -1 and step % RP_DS_freq == 1:
            assert step == prev_loss_i + 1
            parameters = parameters_to_vector(network.parameters())
            update_step = (prev_parameters - parameters) / lr

            RP = get_RP(prev_loss, loss.mean().item(), lr, update_step)
            ds = get_directional_smoothness(prev_update_step, update_step, lr)

            RPs[step // RP_DS_freq] = RP
            directional_smoothness[step // RP_DS_freq] = ds

        if RP_DS_freq != -1 and step % RP_DS_freq == 0:
            prev_loss = loss.mean().item()
            prev_loss_i = step
            prev_update_step = (prev_parameters - parameters_to_vector(network.parameters())) / lr
        ...
        if save_freq != -1 and step % save_freq == 0:
            save_files(directory, [("eigs", eigs[:step // eig_freq]), ("iterates", iterates[:step // iterate_freq]),
                                   ...
                                   ("RPs", RPs[:step // RP_DS_freq]),
                                   ("directional_smoothness", directional_smoothness[:step // RP_DS_freq])])
    ...
    save_files_final(directory,
                     [("eigs", eigs[:(step + 1) // eig_freq]), ("iterates", iterates[:(step + 1) // iterate_freq]),
                      ...
                      ("RPs", RPs[:(step + 1) // RP_DS_freq]),
                      ("directional_smoothness", directional_smoothness[:(step + 1) // RP_DS_freq])])
    if save_model:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--RP_DS_freq", type=int, default=-1,
                        help="the frequency at which we compute RP and DS (-1 means never)")
    ...
    main(dataset=args.dataset, arch_id=args.arch_id, loss=args.loss, opt=args.opt, lr=args.lr, max_steps=args.max_steps,
         ...
         seed=args.seed, RP_DS_freq=args.RP_DS_freq)
