...
def iterate_dataset(dataset: Dataset, batch_size: int, cuda: bool):
    """Iterate through a dataset, yielding batches of data."""
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=False)
    for (batch_X, batch_y) in loader:
        yield batch_X.cuda() if cuda else batch_X, batch_y.cuda() if cuda else batch_y


def compute_losses(network: nn.Module, loss_functions: List[nn.Module], dataset: Dataset,
                   batch_size: int = DEFAULT_PHYS_BS, cuda: bool = True):
    """Compute loss over a dataset."""
    L = len(loss_functions)
    losses = [0. for l in range(L)]
    with torch.no_grad():
        for (X, y) in iterate_dataset(dataset, batch_size, cuda):
            preds = network(X)
...
def compute_hvp(network: nn.Module, loss_fn: nn.Module,
                dataset: Dataset, vector: Tensor, physical_batch_size: int = DEFAULT_PHYS_BS, cuda: bool = False):
    ...
    if cuda:
        hvp = hvp.cuda()
        vector = vector.cuda()
    for (X, y) in iterate_dataset(dataset, physical_batch_size, cuda):
...
def lanczos(matrix_vector, dim: int, neigs: int, cuda: bool):
...
def get_hessian_eigenvalues(network: nn.Module, loss_fn: nn.Module, dataset: Dataset,
                            neigs=6, physical_batch_size=1000, cuda: bool = False):
    """ Compute the leading Hessian eigenvalues. """
    hvp_delta = lambda delta: compute_hvp(network, loss_fn, dataset,
                                          delta, physical_batch_size=physical_batch_size, cuda=cuda).detach().cpu()
    nparams = len(parameters_to_vector((network.parameters())))
    evals, evecs = lanczos(hvp_delta, nparams, neigs=neigs, cuda=cuda)
    return evals
...
def get_RP(prev_loss: float, loss: float, lr: float, update_step: torch.Tensor) -> float:
    return (loss - prev_loss) / (lr * (update_step.norm() ** 2)).item()


def get_directional_smoothness(prev_update_step: torch.Tensor, update_step: torch.Tensor, lr: float) -> float:
    return (prev_update_step.dot(prev_update_step - update_step))/(lr*(prev_update_step.norm()**2)).item()
